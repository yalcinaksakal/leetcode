...
function PriorityQueue(initialArr, comparator) {
	if (!(this instanceof PriorityQueue)) return new PriorityQueue(comparator);
	this.array = initialArr.sort((a, b) => a - b);
...
var minimumTimeRequired = function (jobs, k) {
	if (k >= jobs.length) return Math.max(...jobs);
	jobs.sort((a, b) => b - a);
	const workersLoad = new PriorityQueue(jobs.slice(0, k));
	for (let i = k; i < jobs.length; i++)
		workersLoad.add(workersLoad.poll() + jobs[i]);
	return Math.max(...workersLoad.array);
};
